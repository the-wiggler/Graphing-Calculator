        ...
        for (size_t i = 0; i < SDL_fpoints.size() - 1; i++) {
            ...
            if (!outputs.hasAsymptote) {
                ...
                if (drawLine) {
                    SDL_RenderLine(renderer,  
                        SDL_fpoints[i].x, SDL_fpoints[i].y,      // start point
                        SDL_fpoints[i + 1].x, SDL_fpoints[i + 1].y);  // end pointd
